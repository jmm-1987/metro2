from flask_sqlalchemy import SQLAlchemy
from flask_login import UserMixin
from sqlalchemy import Time
from datetime import datetime

db = SQLAlchemy()

class Usuario(UserMixin, db.Model):
    id = db.Column(db.Integer, primary_key=True)
    nombre = db.Column(db.String(80), unique=True, nullable=False)
    password = db.Column(db.String(200), nullable=False)
    es_admin = db.Column(db.Boolean, default=False)
    email = db.Column(db.String(120), unique=True, nullable=True)
    telefono = db.Column(db.String(20), unique=True, nullable=True)
    # Relación con Evento - backref crea 'usuario' en Evento
    eventos = db.relationship('Evento', backref='usuario', lazy=True)

class Cliente(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    nombre = db.Column(db.String(120), nullable=False)
    email = db.Column(db.String(120), unique=True, nullable=True)
    telefono = db.Column(db.String(20), unique=True)
    temperatura = db.Column(db.Integer, nullable=False, default=1)  # 1 a 5
    estado = db.Column(db.String(20), nullable=False, default='en_curso') # en_curso, ha_comprado, cancelado, ha alquilado
    localidad = db.Column(db.String(120), nullable=True)
    observaciones = db.Column(db.Text, nullable=True)
    tipo_cliente = db.Column(db.String(50), nullable=True)
    interes = db.Column(db.Text, nullable=True)
    precio_min = db.Column(db.Integer, nullable=True)
    precio_max = db.Column(db.Integer, nullable=True)
    encuesta_enviada = db.Column(db.Boolean, default=False)
    comercial_id = db.Column(db.Integer, db.ForeignKey('usuario.id'), nullable=True)
    inmueble_id = db.Column(db.Integer, db.ForeignKey('inmueble.id'), nullable=True)
    fecha_creacion = db.Column(db.DateTime, default=datetime.utcnow)
    # Relación con Evento - backref crea 'cliente' en Evento
    eventos = db.relationship('Evento', backref='cliente', lazy=True)
    visto_por = db.Column(db.Integer, db.ForeignKey('cliente.id'), nullable=True)
    inmuebles_vistos = db.relationship('Inmueble', secondary='inmueble_visto', back_populates='visto_por_clientes')

class Inmueble(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    direccion = db.Column(db.String(200), nullable=False)
    descripcion = db.Column(db.Text)
    categoria = db.Column(db.String(50), nullable=True)
    precio_min = db.Column(db.Integer, nullable=True)
    precio_max = db.Column(db.Integer, nullable=True)
    cliente_id = db.Column(db.Integer, db.ForeignKey('cliente.id'), nullable=True)
    registro_id = db.Column(db.Integer, db.ForeignKey('registro.id'), nullable=True)
    estado = db.Column(db.String(20), nullable=False, default='en_venta')  # en_venta, vendido, cancelado, en_alquiler
    # Relación con Evento - backref crea 'inmueble' en Evento
    eventos = db.relationship('Evento', backref='inmueble', lazy=True)
    visto_por_clientes = db.relationship('Cliente', secondary='inmueble_visto', back_populates='inmuebles_vistos')

class Registro(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    usuario_id = db.Column(db.Integer, db.ForeignKey('usuario.id'))
    cliente_id = db.Column(db.Integer, db.ForeignKey('cliente.id'))
    inmueble_id = db.Column(db.Integer, db.ForeignKey('inmueble.id'), nullable=True)
    fecha = db.Column(db.Date, nullable=False)
    comentario = db.Column(db.Text)

class Evento(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    titulo = db.Column(db.String(120), nullable=False)
    descripcion = db.Column(db.Text)
    fecha = db.Column(db.Date, nullable=False)
    hora_inicio = db.Column(Time, nullable=True)
    hora_fin = db.Column(Time, nullable=True)
    usuario_id = db.Column(db.Integer, db.ForeignKey('usuario.id'))
    cliente_id = db.Column(db.Integer, db.ForeignKey('cliente.id'))
    inmueble_id = db.Column(db.Integer, db.ForeignKey('inmueble.id'), nullable=True)
    # Las relaciones se crean automáticamente a través de los 'backref'
    # en los modelos Usuario, Cliente e Inmueble. 

class RespuestaFormulario(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    cliente_id = db.Column(db.Integer, db.ForeignKey('cliente.id'), nullable=False)
    puntuacion1 = db.Column(db.Integer, nullable=False)
    puntuacion2 = db.Column(db.Integer, nullable=False)
    puntuacion_media = db.Column(db.Float, nullable=False)
    sugerencias = db.Column(db.Text, nullable=True)
    fecha = db.Column(db.DateTime, nullable=False, default=datetime.utcnow) 

inmueble_visto = db.Table('inmueble_visto',
    db.Column('inmueble_id', db.Integer, db.ForeignKey('inmueble.id'), primary_key=True),
    db.Column('cliente_id', db.Integer, db.ForeignKey('cliente.id'), primary_key=True)
) 